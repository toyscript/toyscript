...
tokensWithoutStopWords = [wnl.lemmatize(token) for token in tokens if not token in stopWords]  #  Lemmatization: 표제어 추출
...
# 토큰별 빈도 수 추출
# print(Counter(keyTokens).most_common())

# 빈 문자열 제거
keyTokens = ' '.join([token for token in keyTokens if token])

# 감정 분석
# sia = SentimentIntensityAnalyzer()
# scores = sia.polarity_scores(" ".join(keyTokens))
# print(scores)
